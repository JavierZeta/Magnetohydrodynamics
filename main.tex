...
\author{Víctor Ávila Camargo\\José Ignacio Miguel Rodríguez\\Javier Zaragozano Calvo\\[12pt]
Universidad de Alcalá\\
Facultad de Ciencias\\
Asignatura: Fluidos en Ambientes Planetarios y Espaciales}
\date{\today}

\begin{document}

% Página de portada con imagen\begin{titlepage}
    % Logo en esquina superior izquierda
    \begin{center}
        \includegraphics[height=3cm]{Fotos/uah_logo.png}
    \end{center}
    
    \vspace*{1cm}
    % Título principal
    \begin{center}
        {\Huge\bfseries The Cosmic Engine\par}
        \vspace{0.5cm}
        {\Large\bfseries Fundamentos Magnetohidrodinámicos de la \\ Acreción y la Eyección Relativista\par}
        
        \vspace{1cm}
        \includegraphics[width=0.7\textwidth]{Fotos/portada.jpg}\par
        
        \vspace{0.5cm}
        \small\itshape ``A ring-like accretion structure in M87 
connecting its black hole and jet (2023) \cite{lu2023ring}, 
    \end{center}
    ...
    \begin{center}
        ...
        Javier Zaragozano Calvo \\
        \vspace{0.3cm}
        \small Asignatura: Fluidos en Ambientes Planetarios y Espaciales \\
        \small Departamento de Física y Matemáticas \\
        \small \today
    \end{center}
\tableofcontents
\newpage
...
La MHD puede entenderse, siguiendo a Davidson \cite{Davidson2001}, como una teoría efectiva que emerge de la combinación de la Mecánica de Fluidos clásica con el Electromagnetismo de Maxwell.
\\\\
...
De acuerdo con las hipótesis del modelo MHD \cite{Freidberg2014}, el movimiento del fluido es no
...
denominado \textit{teorema de Alfv\'en} \cite{Alfven1942},   también conocido como el principio de
\textit{flujo congelado}.  Este resultado establece que, en un fluido perfectamente
conductor, el flujo magnético a través de cualquier superficie material se conserva
en el tiempo. Este teorema es matemáticamente análogo al teorema de Kelvin siendo el campo magnético análogo a la vorticidad.
\\\\
Partimos de la ecuación de inducción magnética\eqref{eq:induction} en el límite de conductividad infinita,
$\sigma \to \infty$, donde la difusividad magnética $\eta = 1/(\mu_0 \sigma)$ se anula:
\begin{equation}
\frac{\partial \mathbf{B}}{\partial t}
= \nabla \times (\mathbf{u} \times \mathbf{B}).
\label{eq:induccion_ideal}
\end{equation}

Consideremos una superficie material $S(t)$ que se mueve con el fluido, limitada por
un contorno cerrado $C(t)$. El flujo magnético a través de dicha superficie se define
como
\begin{equation}
\Phi_B(t) = \int_{S(t)} \mathbf{B} \cdot d\mathbf{S}.
\end{equation}

La derivada temporal del flujo magnético viene dada por
\begin{equation}
\frac{d\Phi_B}{dt}
= \int_{S(t)} \frac{\partial \mathbf{B}}{\partial t} \cdot d\mathbf{S}
+ \oint_{C(t)} (\mathbf{u} \times \mathbf{B}) \cdot d\mathbf{l},
\label{eq:flujo_general}
\end{equation}
donde el segundo término tiene en cuenta el movimiento del contorno $C(t)$ con
velocidad $\mathbf{u}$.

Sustituyendo la ecuación de inducción ideal \eqref{eq:induccion_ideal} en el primer
término y utilizando el teorema de Stokes, se obtiene
\begin{align}
\frac{d\Phi_B}{dt}
...
Por tanto, ambos términos se cancelan exactamente y se obtiene
\begin{equation}
\frac{d\Phi_B}{dt} = 0.
\end{equation}
...
Las líneas de campo pueden estirarse y deformarse por el movimiento del plasma, pero la ruptura y la reconexión está fuertemente suprimida a escalas macroscópicas, pero no prohibida. Este principio
...
\begin{figure}[H] 
    \centering
    \includegraphics[width=0.5\textwidth]{Fotos/alfven.png} % Reducido a 0.45 del ancho del texto
    \caption{Representación esquemática del teorema de Alfven} 
    \label{alfven}
\end{figure}
...
intuitiva mediante una analogía introducida por Balbus \& Hawley \cite{Balbus1991a}, que
...
en un radio $R_0$, conocido como \textit{shearing sheet} \cite{Hawley1991a}. En este marco,
las coordenadas son
\begin{equation}
...
Analizando el término independiente de la relación de dispersión, se observa que el
factor crítico es
\begin{equation}
k_z^2 v_A^2 + \frac{d\Omega^2}{d\ln R}.
\end{equation}

Dado que $k_z^2 v_A^2 > 0$, la existencia de una raíz inestable ($\omega^2 < 0$)
...
\begin{equation}
\frac{d\Omega^2}{d\ln R} = 2\Omega R \frac{d\Omega}{dR},
\end{equation}
y dado que $\Omega > 0$ en discos de acreción, esta condición es equivalente a
\begin{equation}
\frac{d\Omega}{dR} < 0.
...
Si el campo es excesivamente intenso, la tensión magnética rigidiza el fluido (haciendo que la velocidad de Alfvén $v_A$ sea muy alta) y suprime los desplazamientos relativos; matemáticamente, esto ocurre cuando $k_z^2 v_A^2$ supera el valor absoluto del gradiente de velocidad angular. 
...
\end{equation}
que actúan como el mecanismo efectivo de transporte de momento angular que se buscaba en modelos fenomenológicos como el de Shakura \& Sunyaev \cite{Shakura1973}. Estos esfuerzos permiten que la materia pierda momento angular y caiga hacia el objeto central, resolviendo de manera natural el problema de la viscosidad. 
...
De manera general, el mecanismo de funcionamiento de un dinamo nos permite transformar energía mecánica en magnética. Una manera sencilla de entender cómo la turbulencia actúa como un dinamo rápido (un dinamo rápido es un dinamo que gira a altas revoluciones) es mediante el llamado mecanismo Stretch-Twist-Fold (STF) \cite{Zeldovich1983} introducido por el físico bieloruso Yákov Zeldóvich. 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\linewidth]{Fotos/STF.png}
    \caption{Mecanismo Stretch-Twist-Fold}
\end{figure}
Este modelo refleja de una manera muy clara las claves necesarias para que una dinamo sea eficiente. Estas claves son:
\begin{description}
    ...
    \item[3.] Procesos difusivos para hacer el ciclo irreversible.
\end{description}
...
La teoría de dinamos de campo medio \cite{Brandenburg2005} pemite entender cómo los campos magnéticos grandes y coherentes pueden generarse a partir de la interacción de campos turbulentos más pequeños.

En una primera instancia, Eugene Parker propuso la idea de que un campo magnético poloidal que surja a partir de las fuerzas de Coriolis, las cuales pueden convertir un campo toroidal en poloidal, se podía describir introduciendo un término en la ecuación de inducción.
\begin{equation}
    \frac{\partial\langle\vect{B}_{p}\rangle}{\partial t}=\vect{\nabla}\times(\alpha\langle\vect{B}_{tor}\rangle+\dots)
\end{equation}
Esto solo es aplicable a los campos medios porque la fuerza electromagnética inducida ($\vect{u}\times\vect{B}$) nunca tendría una componente paralela al campo magnético. Recordemos que en la teoría de campos medios, las variables se descomponen en la suma del valor y de las fluctuaciones, al igual que en los flujos turbulentos. Así:
\begin{align*} 
...
A partir del crecimiento exponencial del campo toroidal, por el efecto $\alpha^{2}$ este campo hace crecer también el campo poloidal. Es precisamente este campo el que en gran parte provoca la aparición de los jets en los discos de acreción.
\newpage
\begin{figure}[h]
    \centering
...
Cuando la teoría de dinamos se aplicó a los discos de acreción, inicialmente se consideraba un ejemplo más de los numerosos cuerpos astrofísicos que albergan dinamos. Sin embargo, con el descubrimiento de la inestabilidad magnetorotacional se hizo evidente que los campos magnéticos son esenciales para sostener la turbulencia en los discos de acreción. Esto da lugar a un sistema autoexcitado en el que el campo magnético necesario para la MRI puede regenerarse continuamente mediante la acción del dinamo.
